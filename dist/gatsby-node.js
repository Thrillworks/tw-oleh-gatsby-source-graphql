...
exports.sourceNodes = async ({
  ...
  invariant(typeName && typeName.length > 0, `tw-oleh-gatsby-source-graphql requires option \`typeName\` to be specified`);
  invariant(fieldName && fieldName.length > 0, `tw-oleh-gatsby-source-graphql requires option \`fieldName\` to be specified`);
  invariant(url && url.length > 0 || createLink, `tw-oleh-gatsby-source-graphql requires either option \`url\` or \`createLink\` callback`);
  let link;
...
exports.onPreExtractQueries = async ({
  ...
  const CACHE_DIR = path.resolve(`${program.directory}/.cache/thrillworks/assets/`);
...
exports.createResolvers = ({
  ...
  Object.keys(typeMap).forEach(typeName => {
    ...
    Object.keys(typeFields).forEach(fieldName => {
      ...
      if (field.type && String(field.type).includes('prodigyapi') && sharpKeys.some(re => re instanceof RegExp ? re.test(fieldName) : re === fieldName)) {
        // console.log(field);
