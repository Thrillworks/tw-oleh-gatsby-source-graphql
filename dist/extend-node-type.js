...
const THRILLWORKS_IMAGE_MAX_SIZE = 4000;
...
const createUrl = (imgUrl, options = {}) => {
  // Convert to Thrillworks names and filter out undefined/null values.
  const args = _.pickBy({
    w: options.width,
    h: options.height,
...
const resolveFixed = (image, options) => {
  ...
  fixedSizes = fixedSizes.map(Math.round); // Filter out sizes larger than the image's width and the Thrillworks image's max size.

  const filteredSizes = fixedSizes.filter(size => {
    const calculatedHeight = Math.round(size / desiredAspectRatio);
    return size <= THRILLWORKS_IMAGE_MAX_SIZE && calculatedHeight <= THRILLWORKS_IMAGE_MAX_SIZE && size <= width;
...
const resolveFluid = (image, options) => {
  ...
  fluidSizes = fluidSizes.map(Math.round); // Filter out sizes larger than the image's maxWidth and the Thrillworks image's max size.
  ...
  if (!filteredSizes.includes(parseInt(width)) && parseInt(width) < THRILLWORKS_IMAGE_MAX_SIZE && Math.round(width / desiredAspectRatio) < THRILLWORKS_IMAGE_MAX_SIZE) {
...
exports.extendNodeType = ({
  ...
  const fixedNode = fixedNodeType({
    name: `ThrillworksFixed`,
    getTracedSVG
  });
  const resolutionsNode = fixedNodeType({
    name: `ThrillworksResolutions`,
    getTracedSVG
  });
  ...
  const fluidNode = fluidNodeType({
    name: `ThrillworksFluid`,
    getTracedSVG
  });
  const sizesNode = fluidNodeType({
    name: `ThrillworksSizes`,
    getTracedSVG
  });
