...
const bar = new ProgressBar(
  `Downloading Thrillworks Assets [:bar] :current/:total :elapsed secs :percent`,
  {
    total: 0,
    width: 30,
  },
...
const downloadThrillworksAsset = async gatsbyFunctions => {
  ...
  // console.log('get nodes for downloading', getNodes());
  ...
  const thrillworksAssetNodes = getNodes().filter(
    n => n.internal.owner === `tw-oleh-gatsby-source-graphql`,
  );

  // console.log(thrillworksAssetNodes);
  await Promise.all(
    thrillworksAssetNodes.map(async node => {
      totalJobs += 1;
      bar.total = totalJobs;
