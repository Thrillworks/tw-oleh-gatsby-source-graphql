...
const resolveFixed = (image, options) => {
  ...
  // Filter out sizes larger than the image's width and the Thrillworks image's max size.
  const filteredSizes = fixedSizes.filter(size => {
    const calculatedHeight = Math.round(size / desiredAspectRatio);
    return (
      size <= THRILLWORKS_IMAGE_MAX_SIZE &&
      calculatedHeight <= THRILLWORKS_IMAGE_MAX_SIZE &&
      size <= width
    );
  });
...
const resolveFluid = (image, options) => {
  ...
  // Filter out sizes larger than the image's maxWidth and the Thrillworks image's max size.
  ...
  if (
    ...
    parseInt(width) < THRILLWORKS_IMAGE_MAX_SIZE &&
    Math.round(width / desiredAspectRatio) < THRILLWORKS_IMAGE_MAX_SIZE
  ) {
...
exports.extendNodeType = ({ type, store }) => {
  ...
  const fixedNode = fixedNodeType({ name: `ThrillworksFixed`, getTracedSVG });
  ...
  const fluidNode = fluidNodeType({ name: `ThrillworksFluid`, getTracedSVG });
  const sizesNode = fluidNodeType({ name: `ThrillworksSizes`, getTracedSVG });
